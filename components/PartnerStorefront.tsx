  ...
  useEffect(() => {
    ...
    async function carregar() {
      try {
        setCarregando(true);
        setErro("");
        const resposta = await fetch(`/api/economize/ofertas?loja=${encodeURIComponent(slug)}`, { cache: "no-store" });
        const dados = (await resposta.json()) as Resposta;
        if (!resposta.ok) throw new Error(dados.error || `Não foi possível carregar ${nome}.`);
        if (ativo) setOfertas(dados.ofertas || []);
      } catch (error) {
        if (ativo) setErro(error instanceof Error ? error.message : `Erro ao carregar ${nome}.`);
      } finally {
  ...
  return (
    <main className="min-h-screen bg-zinc-100 text-zinc-950">
      <header className="border-b border-zinc-200 bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4">
          ...
          <div className="flex gap-2">
            ...
            <Link href="/" className="rounded-full px-4 py-2 text-sm font-black text-white" style={{ backgroundColor: cor }}>Início</Link>
          </div>
        </div>
      </header>
      ...
      <section className="bg-[#101010] text-white">
        <div className="mx-auto grid max-w-7xl gap-8 px-5 py-14 lg:grid-cols-[1.2fr_0.8fr] lg:items-end lg:py-20">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="rounded-2xl border border-white/15 bg-white/5 p-4"><p className="text-xs font-bold text-zinc-400">Cupons</p><p className="mt-2 text-3xl font-black">{cupons}</p></div>
      ...
      <section className="mx-auto max-w-7xl px-5 py-8">
        ...
        {carregando ? (
          <div className="mt-6 rounded-3xl bg-white p-12 text-center font-black shadow-sm">Carregando ofertas {nome}...</div>
        ) : erro ? (
