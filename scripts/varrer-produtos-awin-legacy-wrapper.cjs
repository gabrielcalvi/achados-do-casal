...
const diagnosticoCallback = `    await lerCsvStreaming(stream, delimitador, (row) => {\n      total += 1;\n      const moedaDiag = texto(campo(row, ["currency", "moeda"])) || "BRL";\n      const precoAtualDiag = parsePreco(campo(row, ["search_price", "store_price", "base_price", "price", "sale_price"]), moedaDiag);\n      const precoOriginalDiag = parsePreco(campo(row, ["rrp_price", "product_price_old", "old_price", "was_price"]), moedaDiag);\n\n      if (precoAtualDiag && precoOriginalDiag && precoAtualDiag.moeda === "BRL" && precoOriginalDiag.moeda === "BRL") {\n        const diferenca = precoOriginalDiag.valor - precoAtualDiag.valor;\n        if (diferenca > 0.009) {\n          diagnostico.preco_original_maior += 1;\n          const pctReal = ((precoOriginalDiag.valor - precoAtualDiag.valor) / precoOriginalDiag.valor) * 100;\n          if (pctReal >= 10) diagnostico.desconto_10_mais += 1;\n          if (pctReal > diagnostico.desconto_maximo_percentual) diagnostico.desconto_maximo_percentual = Math.round(pctReal * 10) / 10;\n        } else if (Math.abs(diferenca) <= 0.009) {\n          diagnostico.preco_original_igual += 1;\n        } else {\n          diagnostico.preco_original_menor += 1;\n        }\n      }\n\n      if (diagnostico.amostras < 5000) {\n        diagnostico.amostras += 1;\n        if (!diagnostico.cabecalhos.length) diagnostico.cabecalhos = Object.keys(row).slice(0, 120);\n        if (texto(campo(row, ["aw_product_id", "merchant_product_id", "product_id", "id"]))) diagnostico.com_id += 1;\n        if (texto(campo(row, ["product_name", "name", "title"]))) diagnostico.com_titulo += 1;\n        if (urlValida(campo(row, ["merchant_deep_link", "merchant_link", "product_url"]), loja.dominio)) diagnostico.com_link_mercante += 1;\n        if (texto(campo(row, ["merchant_deep_link", "merchant_link", "product_url", "deep_link", "aw_deep_link", "awin_deep_link"]))) diagnostico.com_link_qualquer += 1;\n        if (imagemValida(campo(row, ["merchant_image_url", "image_url", "image_link", "large_image", "aw_image_url", "merchant_thumb_url", "merchant_thumb"]))) diagnostico.com_imagem += 1;\n        if (precoAtualDiag) diagnostico.com_preco_atual += 1;\n        if (precoOriginalDiag) diagnostico.com_preco_original += 1;\n        if (parsePreco(campo(row, ["saving", "savings"]), moedaDiag)) diagnostico.com_saving += 1;\n        const pct = numero(campo(row, ["savings_percent", "saving_percent", "discount_percent"]));\n        if (pct !== null && pct > 0) diagnostico.com_percentual += 1;\n      }\n      const produto = normalizarProdutoLegacy(row, loja);`;
...
const feedComDiagnostico = `      item.selecionados = feed.selecionados.length;\n      item.diagnostico = feed.diagnostico || null;\n      if (item.diagnostico) {\n        const d = item.diagnostico;\n        console.log(\`[AWIN diag] loja=\${loja.slug} amostras=\${d.amostras} id=\${d.com_id} titulo=\${d.com_titulo} linkMercante=\${d.com_link_mercante} linkQualquer=\${d.com_link_qualquer} imagem=\${d.com_imagem} precoAtual=\${d.com_preco_atual} precoOriginal=\${d.com_preco_original} saving=\${d.com_saving} percentual=\${d.com_percentual} rrpMaior=\${d.preco_original_maior} rrpIgual=\${d.preco_original_igual} rrpMenor=\${d.preco_original_menor} desconto10=\${d.desconto_10_mais} descontoMax=\${d.desconto_maximo_percentual}\`);\n        console.log(\`[AWIN diag headers] loja=\${loja.slug} \${(d.cabecalhos || []).join(',')}\`);\n      }`;
