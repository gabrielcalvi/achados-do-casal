...
async function extrairCea(page, urlProduto) {
  ...
  const dados = await page.evaluate(() => {
    ...
    function obterImagem(elemento) {
      ...
      return normalizarImagem(
        elemento.currentSrc ||
          ...
          elemento.getAttribute("data-original") ||
          elemento.getAttribute("data-lazy-src") ||
          ""
      );
    }
    ...
    const precoAtualTexto = textoDoPrimeiro([
      '[itemprop="price"]',
      '[data-testid="price"]',
    ...
    const precosAnteriores = textosDosSeletores([
      "del",
      "s",
    ...
    const precoDePor =
      corpo.match(
        /de\s+R\$\s*([\d.]+,\d{2})\s+(?:por|a partir)/i
      );
