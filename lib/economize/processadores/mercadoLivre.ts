...
function numerosIguais(
  ...
  const numeroPrimeiro =
    numeroOuNull(primeiro);

  const numeroSegundo =
    numeroOuNull(segundo);
...
function ofertaFoiAlterada(
  existente: OfertaExistente,
  extraida: OfertaMercadoLivreExtraida
) {
  if (
    !textosIguais(
      existente.titulo,
  ...
  if (
    !textosIguais(
      existente.link_destino,
      extraida.link_destino
    )
  ) {
    return true;
  }
  ...
  if (
    !numerosIguais(
      existente.preco_original,
      extraida.preco_original
    )
  ) {
    return true;
  }
  ...
  if (
    !numerosIguais(
      existente.preco_oferta,
      extraida.preco_oferta
    )
  ) {
    return true;
  }
  ...
  if (
    !numerosIguais(
      existente.desconto_percentual,
      extraida.desconto_percentual
    )
  ) {
    return true;
  }
...
async function baixarHtml(
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `A fonte retornou um conteúdo inesperado: ${contentType}.`
      );
    }
    ...
    if (
      ...
      throw new Error(
        "A consulta ao Mercado Livre excedeu 20 segundos."
      );
    }
...
async function carregarOfertasExistentes(
  lojaId: string,
  chaves: string[]
) {
  if (chaves.length === 0) {
  ...
  if (error) {
    throw new Error(
      `Não foi possível consultar as ofertas existentes: ${error.message}`
    );
  }

  return (
    (data ?? []) as OfertaExistente[]
  );
}
...
export async function processarFonteMercadoLivre(
  ...
  if (!fonte.ativa) {
    throw new Error(
      "A fonte do Mercado Livre está inativa."
    );
  }
  ...
  const html = await baixarHtml(
    fonte.url
  );

  const ofertasExtraidas =
    extrairOfertasMercadoLivre(
      html,
      fonte.url
    );
  ...
  if (ofertasExtraidas.length === 0) {
    throw new Error(
      "Nenhuma oferta válida foi encontrada no HTML recebido."
    );
  }

  const chaves = ofertasExtraidas.map(
    (oferta) => oferta.dedupe_key
  );

  const ofertasExistentes =
    await carregarOfertasExistentes(
      fonte.loja_id,
      chaves
    );

  const mapaExistentes = new Map(
    ofertasExistentes.map((oferta) => [
      oferta.dedupe_key,
      oferta,
    ])
  );
  ...
  if (novas.length > 0) {
    ...
    if (erroInsercao) {
      throw new Error(
        `Não foi possível inserir as novas ofertas: ${erroInsercao.message}`
      );
    }
  ...
  const ofertasParaAtualizar =
    ofertasExtraidas.filter(
      (oferta) =>
        mapaExistentes.has(
          oferta.dedupe_key
        )
  ...
  for (
    ...
    if (erroAtualizacao) {
      erros.push(
        `${oferta.titulo}: ${erroAtualizacao.message}`
      );
...
export function mensagemErroProcessamentoMercadoLivre(
  error: unknown
) {
