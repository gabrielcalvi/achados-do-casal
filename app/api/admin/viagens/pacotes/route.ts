...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    if (!id) {
      return NextResponse.json(
        { sucesso: false, erro: "Pacote invalido para edicao." },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({ sucesso: true, pacote: data });
  } catch (erro) {
    console.error("[Pacotes viagem] Falha ao editar:", erro);
    ...
    return NextResponse.json(
      {
        ...
        erro:
          erro instanceof Error
            ? erro.message
            : "Erro inesperado ao editar pacote.",
      },
      { status: 500 }
    );
  }
