...
async function status(request: NextRequest) {
  ...
  try {
    ...
    const dados = (await lerJson(sandbox, STATUS_PATH)) || (await lerJson(sandbox, RESULT_PATH)) || { executando: false, mensagem: "Nenhuma varredura Nike Seguro executada ainda." };
...
async function executar(request: NextRequest) {
  ...
  if (!awinToken || !datafeedKey || !supabaseUrl || !serviceKey) {
    return NextResponse.json({ sucesso: false, erro: "Variaveis AWIN/Data Feed/Supabase incompletas.", datafeed_configurado: Boolean(datafeedKey) }, { status: 500 });
  }

  try {
    const sandbox = await Sandbox.get({ name: SANDBOX_NAME });
    const preparar = await comando(sandbox, "mkdir", ["-p", "/vercel/scripts", "/vercel/tmp"]);
    if (preparar.resultado.exitCode !== 0) throw new Error(preparar.stderr || "Falha preparando diretorios do Sandbox.");

    const commit = process.env.VERCEL_GIT_COMMIT_SHA?.trim() || "main";
    const arquivos = [
      [`https://raw.githubusercontent.com/${REPOSITORY}/${encodeURIComponent(commit)}/scripts/varrer-produtos-awin-legacy.cjs`, BASE_SCRIPT],
      [`https://raw.githubusercontent.com/${REPOSITORY}/${encodeURIComponent(commit)}/scripts/varrer-produtos-awin-nike-seguro.cjs`, NIKE_SAFE_SCRIPT],
      [`https://raw.githubusercontent.com/${REPOSITORY}/${encodeURIComponent(commit)}/scripts/varrer-produtos-awin-nike-ampliado.cjs`, NIKE_SCRIPT],
    ...
    return NextResponse.json({
      sucesso: true,
      iniciado: true,
      modo: "nike_seguro_melhores_ofertas",
      advertiser_id: 17652,
      candidatos_internos: 120,
      ...
      ordenacao: "desconto_desc_economia_desc",
