...
async function processoAtivo(sandbox: SandboxInstancia) {
  const processos = await comando(sandbox, "ps", ["-eo", "pid=,args="]);
  if (processos.resultado.exitCode !== 0) return false;
  return processos.stdout.split("\n").some((linha) => linha.includes("awin-produtos-casas-bahia") || linha.includes("varrer-produtos-awin-legacy-wrapper.cjs"));
}
...
async function status(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ sucesso: true, status: dados });
  } catch (erro) {
...
async function executar(request: NextRequest) {
  ...
  try {
    ...
    await comando(sandbox, "sed", ["-i", "s#AbortSignal.timeout(240000)#AbortSignal.timeout(900000)#g", SCRIPT_PATH]);
    await comando(sandbox, "sed", ["-i", `s#/vercel/tmp/awin-produtos-status.json#${STATUS_PATH}#g`, SCRIPT_PATH]);
    await comando(sandbox, "sed", ["-i", `s#/vercel/tmp/awin-produtos-resultado.json#${RESULT_PATH}#g`, SCRIPT_PATH]);

    const anterior = await lerJson(sandbox, STATUS_PATH);
    if (anterior?.executando === true && await processoAtivo(sandbox)) {
    ...
    const env = {
      ...
      CASAS_BAHIA_AWIN_ADVERTISER_ID: "17629",
      AWIN_PRODUTOS_LOJAS: "casas-bahia",
      AWIN_PRODUTOS_LIMITE_POR_LOJA: "120",
      AWIN_PRODUTOS_DESCONTO_MINIMO: "10",
      AWIN_PRODUTOS_CATALOGO_LOJAS: "casas-bahia",
    };
    ...
    return NextResponse.json({
      ...
      advertiser_id: "17629",
      limite: 120,
      catalogo_sem_desconto_inventado: true,
