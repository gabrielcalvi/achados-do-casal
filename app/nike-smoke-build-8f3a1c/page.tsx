...
async function disparar() {
  ...
  if (!awinToken || !datafeedKey || !supabaseUrl || !serviceKey) {
    throw new Error("Variaveis AWIN/Data Feed/Supabase incompletas no preview.");
  }
  ...
  for (const [url, destino] of arquivos) {
    ...
    if (resultado.exitCode !== 0) {
      throw new Error(`Falha sincronizando ${destino}.`);
    }
  }
  ...
  console.log("[NIKE SMOKE BUILD] Varredura Nike Seguro iniciada no Sandbox.");
  return "iniciado";
}

export default async function NikeSmokeBuildPage() {
  const status = await disparar();
  return <main>nike smoke: {status}</main>;
}
