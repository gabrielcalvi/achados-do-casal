...
async function carregarRadar(
  ...
  try {
    const baseUrl =
      ...
      "achadosdocasal.com.br";

    const origem = baseUrl.startsWith("http")
      ? baseUrl
      : `https://${baseUrl}`;
    ...
    if (!resposta.ok || !dados?.sucesso) {
      throw new Error(
        dados?.erro || "Não foi possível carregar o radar."
      );
    }
    ...
    return {
      ...
      ultimaAtualizacao:
        typeof dados.ultimaAtualizacao === "string"
          ? dados.ultimaAtualizacao
          : null,
      melhorPreco: Number.isFinite(
        Number(dados.melhor?.precoPorPessoa)
      )
        ? Number(dados.melhor.precoPorPessoa)
        : null,
      moeda: String(dados.melhor?.moeda || "BRL"),
      faixa:
        typeof dados.melhor?.tituloFaixa === "string"
          ? dados.melhor.tituloFaixa
          : null,
    };
  } catch (error) {
    return {
          ...
          <div className="mt-5 overflow-x-auto rounded-2xl border border-slate-200">
            <table className="min-w-full text-left text-sm">
              <thead className="bg-slate-50 text-slate-600">
                <tr>
                  <th className="px-4 py-3 font-black">Radar</th>
                  <th className="px-4 py-3 font-black">Rota</th>
                  <th className="px-4 py-3 font-black">Observações</th>
                  <th className="px-4 py-3 font-black">Melhor preço</th>
                ...
                {radares.map((radar) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-bold">
                      {radar.origem} → {radar.destino}
                    </td>
                    <td className="px-4 py-3">
                      {radar.observacoes.toLocaleString("pt-BR")}
                    </td>
